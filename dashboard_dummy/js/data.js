...
function buildEntityCache(allEntities) {
  return allEntities
    .map(row => ({
      ...
      name: row.municipality_csv,
      type: row.muni_org,
      fy25_disbursement: row.agonumbers,
      carryover: row.ro_funds,
      total_available: (row.agonumbers || 0) + (row.ro_funds || 0),
      total_expended: (row.t_exp || 0) + (row.t_exp_v2 || 0),
      total_encumbered: (row.t_enc || 0) + (row.t_enc_v2 || 0),
    ...
    .map(row => ({
      ...row,
      total_remaining: row.total_available - row.total_expended - row.total_encumbered,
      pct_spent:
        row.total_available > 0
          ? (((row.total_expended + row.total_encumbered) / row.total_available) * 100).toFixed(1)
          : 0
    }));
}
 ...
 */
function buildHistoricalCache(tableName, keyField, label) {
  const cache = new Map();
  try {
    const rows = query(`SELECT * FROM ${tableName}`);
    rows.forEach(row => {
      cache.set(row[keyField], row);
    });
...
async function populateCaches() {
  ...
  // Build historical data caches
  fy23EntityCache = buildHistoricalCache('municipal_data_fy23', 'municipality', 'FY2023');
  fy24EntityCache = buildHistoricalCache('municipal_data_fy24', 'municipality', 'FY2024');
    ...
    rize:
      rizeTotal > 0
        ? {
          ...
          source: 'rize_grants.json',
          label: 'RIZE Municipal Matching'
        }
        : null,
    expended: {
      value: expended,
      source: 't_exp + t_exp_v2',
      label: 'Expended'
    },
    utilized: {
      value: totalAvailable > 0 ? (((expended + encumbered) / totalAvailable) * 100).toFixed(1) : '0',
      source: 'calculated: (expended + encumbered) / total_available',
      label: 'Utilized'
    },
    fiscalYear: 2025,
    sourceFile: 'opioid_settlement.db (municipal_data)',
...
function getHeroDataFy24(municipalityName) {
  ...
  return {
    disbursement: {
      ...
      source: 'FY2024_FUNDS_RECEIVED__C',
      label: 'FY24 Disbursement'
    },
    carryover: {
      value: entity.carryover_funds,
      source: 'CARRYOVER_FUNDS_FROM_PREVIOUS_YEARS__C',
      label: 'Carryover Funds'
    },
    expended: {
      value: expended,
      source: 'TOTALABATEMENTFUNDSEXPENDED__C',
      label: 'Expended'
    },
    utilized: {
      value: totalAvailable > 0 ? ((expended / totalAvailable) * 100).toFixed(1) : '0',
      source: 'calculated: expended / total_available',
      label: 'Utilized'
    },
    fiscalYear: 2024,
    sourceFile: entity.source_file || 'FY24MunicipalOpioidAbatement_RawData.csv',
    surveyPath: entity.survey_md_path,
    unexpended: {
      value: entity.unexpended_funds,
      source: 'TOTAL_UNEXPENDED_FUNDS_REMAINING__C'
    }
  };
}
...
function getHeroDataFy23(municipalityName) {
  ...
  return {
    ...
    expended: {
      ...
      source: 'Funds Expended',
      label: 'Expended'
    },
    utilized: {
      value: entity.pct_expended || '0%',
      source: 'Percentage Expended',
      label: 'Utilized'
    },
    fiscalYear: 2023,
    sourceFile: entity.source_file || 'Muni Raw Data_FY23.csv',
