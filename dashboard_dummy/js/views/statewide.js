    ...
    container.innerHTML = `
            ...
            <div class="card" style="background: linear-gradient(135deg, var(--color-bg-secondary) 0%, var(--color-bg) 100%); border-left: 4px solid var(--color-primary);">
                ...
                <div style="display: flex; gap: 2rem; flex-wrap: wrap; margin-top: 0.5rem;">
                    <div>
                        <div style="font-size: 1.5rem; font-weight: 600; color: var(--color-text);">$${Data.formatCurrency(stateSummary.total)}</div>
                        <div style="font-size: 0.875rem; color: var(--color-text-muted);">from CTHRU (FY25)</div>
                    </div>
                    <div>
                        <div style="font-size: 1.5rem; font-weight: 600; color: var(--color-text);">${stateSummary.uniqueVendors}</div>
                        <div style="font-size: 0.875rem; color: var(--color-text-muted);">Vendors Paid</div>
                    </div>
                    <div>
                        <div style="font-size: 1.5rem; font-weight: 600; color: var(--color-text);">${stateSummary.recordCount.toLocaleString()}</div>
                        <div style="font-size: 0.875rem; color: var(--color-text-muted);">Transactions</div>
                    </div>
                </div>
                <p style="color: var(--color-text-muted); margin-top: 1rem; margin-bottom: 0; font-size: 0.875rem;">
  ...
  renderTopSpenders(spenders) {
    ...
    container.innerHTML = spenders
      .map((entity, i) => {
        ...
        return `
                <div class="spender-bar-row" style="margin-bottom: 0.75rem; cursor: pointer;" 
                     data-record-id="${entity.record_id}"
                     tabindex="0"
                     role="button"
                     aria-label="View ${entity.name} details">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
                        <span style="font-weight: 500; font-size: 0.875rem;">${i + 1}. ${entity.name}</span>
                        <span style="color: var(--color-text-muted); font-size: 0.875rem;">$${Data.formatCurrency(total)}</span>
                    </div>
                    <div style="background: var(--color-bg-secondary); border-radius: 4px; height: 20px; overflow: hidden; display: flex;">
