...
function getEntityType(entity) {
  ...
  if (entity.name && entity.name.endsWith('County')) {return 'county';}
  return 'municipality';
}
...
export default {
                ...
                <div class="card-header">
                    ...
                    <div style="display: flex; gap: 1rem; align-items: center;">
                        ...
                        <input type="text"
                               ...
                               class="search-input"
                               style="width: 250px; padding: 0.5rem 1rem; border: 1px solid var(--color-border); border-radius: 8px;"
  ...
  renderTable(entities) {
    ...
    filtered.sort((a, b) => {
      ...
      // Handle nulls
      if (aVal == null) {aVal = sortColumn === 'name' ? '' : 0;}
      if (bVal == null) {bVal = sortColumn === 'name' ? '' : 0;}
    ...
    tbody.innerHTML = filtered.map(entity => {
      ...
      const typeBadge = entityType === 'county' ? '<span class="badge" style="margin-left: 0.5rem; background: var(--color-bg-secondary);">County</span>' :
        entityType === 'organization' ? '<span class="badge" style="margin-left: 0.5rem; background: #E8D5E8;">Org</span>' : '';
      ...
      return `
                <tr class="clickable" 
                    ...
                    tabindex="0" 
                    role="row"
                    aria-label="View ${escapeHtml(entity.name)} details">
                    <td>
                        <strong>${escapeHtml(entity.name)}</strong>
                        ${typeBadge}
                        ${entity.is_collaborative === 1 ? '<span class="badge badge-encumbered" style="margin-left: 0.5rem;">OAC</span>' : ''}
                    </td>
                    ...
                    <td class="text-right">
                        <span class="badge ${pctClass}">${entity.pct_spent}%</span>
                    </td>
