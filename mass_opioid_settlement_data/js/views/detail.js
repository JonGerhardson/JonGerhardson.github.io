...
export default {
  ...
  async render(container, db, params) {
    ...
    const displayName = formatMunicipalityName(municipalityName);
      ...
      detailedContent = `
                <div id="fy24-survey-container" class="fy24-survey-wrapper">
                    <div class="fy24-header mb-lg">
                        ...
                        <h2 style="display: inline; margin-left: var(--space-sm);">Full Expenditure Report</h2>
                    </div>
    ...
    return `
            <div class="hero-stats mb-lg">
                ...
                ${heroData.carryover.value !== null ? `
                    <div class="card stat-card${heroData.carryover.warning ? ' stat-card--warning' : ''}">
                        <h3>${escapeHtml(heroData.carryover.label)}${heroData.carryover.warning ? ' <span class="stat-warning" title="' + escapeHtml(heroData.carryover.warning) + '">!</span>' : ''}</h3>
                        <div class="stat-value">${Data.formatCurrency(heroData.carryover.value)}</div>
                        ${heroData.carryover.warning ? `<div class="debug-info debug-info--warning">${escapeHtml(heroData.carryover.warning)}</div>` : ''}
                ...
                <div class="card stat-card">
                    ...
                    <div class="stat-value">${Data.formatPercent(heroData.utilized.value)}</div>
  ...
  renderFy25DetailedContent() {
    ...
    if (this.entity.annual_municipal_expenditure_report_2_complete == 0) {
      html += `
                <div class="card mb-lg" style="border-left: 4px solid var(--color-error); background-color: var(--color-error-bg);">
                    <div style="display: flex; gap: var(--space-md); align-items: start;">
                        <span style="font-size: var(--font-size-2xl);">⚠️</span>
                        <div>
                            <h3 style="margin-top: 0; margin-bottom: var(--space-sm); color: var(--color-error);">
                                Report Incomplete
    ...
    try {
      ...
      if (!dataRes.ok) {
        if (dataRes.status === 404) {
          contentDiv.innerHTML = `<p>No FY2024 report found for ${escapeHtml(formatMunicipalityName(municipalityName))}.</p>`;
          return;
        }
      ...
      html += `
                <div class="mb-lg" style="text-align: right;">
                    <a href="${escapeHtml(data.submission_url)}" target="_blank" class="btn btn-sm btn-outline-primary">
                        <span style="margin-right: var(--space-sm);">📄</span> Download original filing from mosiac.rizema.org
                    </a>
                </div>
            `;
    }
  ...
  renderSection(section, data, getValue) {
    ...
    // Section Title
    if (section.title) {
      html += `<h3 class="survey-section-title mb-lg" style="border-bottom: 2px solid var(--color-border); padding-bottom: var(--space-sm);">${escapeHtml(section.title)}</h3>`;
    }
  ...
  renderGridTable(gridData, fieldKey) {
    ...
    schema.columns.forEach(col => {
      html += `<th class="text-center" style="font-size: var(--font-size-xs);">${escapeHtml(col.label)}</th>`;
    });
    html += '</tr></thead>';
  ...
  renderStrategies(groups, data, getValue) {
    ...
    groups.forEach(group => {
      ...
      html += `<div class="strategy-card card mb-md p-md ${borderClass} ${opacityClass}" style="border-left: 5px solid ${isNoWork ? 'var(--color-border)' : 'var(--color-success)'};">`;
    ...
    return `
            <div class="card mb-lg">
                ...
                <div class="project-list">
                    ${projects.map(proj => `
                        <div style="padding: var(--space-md); background: var(--color-bg-primary); border-radius: var(--border-radius-sm); margin-bottom: var(--space-sm);">
                            <strong>${escapeHtml(proj.name)}</strong>
                            ${proj.sourceKeys ? `<span class="debug-info">${proj.sourceKeys.name}</span>` : ''}
                            <span class="badge" style="margin-left: var(--space-sm); background: var(--color-bg-secondary);">${proj.source}</span>
                            
                            ${proj.description ? `
                                <div style="margin-top: var(--space-sm); color: var(--color-text-muted); font-size: var(--font-size-sm);">
  ...
  renderExpenditures(expenditures, reconciliation) {
    const discrepancyHtml = reconciliation && reconciliation.hasDiscrepancy ? `
            <p style="color: var(--chart-2); font-size: var(--font-size-sm); margin-bottom: var(--space-md);">
                        ...
                        <tbody>
                            ${expenditures.map(exp => `
                                <tr>
                                    ...
                                    <td style="max-width: 300px; font-size: var(--font-size-sm);">
                                        <div style="margin-bottom: var(--space-xs);">
                                            ${escapeHtml(exp.description)}
                                            ${exp.sourceKeys ? `<span class="debug-info">${exp.sourceKeys.description}</span>` : ''}
                                        </div>
                                        ${exp.notes ? `
                                            <div style="color: var(--color-text-muted); font-style: italic; border-top: 1px dashed var(--color-border); padding-top: var(--space-xs); margin-top: var(--space-xs);">
  ...
  renderRizeGrants(grants) {
    ...
    return `
            <div class="card mb-lg rize-grants-section">
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-md);">
                    <h3 style="margin: 0;">
  ...
  renderCoreGrants(grants) {
    return `
            <div class="mb-lg">
                <h4 class="mb-md" style="color: var(--color-mosaic); border-bottom: 2px solid var(--color-mosaic-light); padding-bottom: var(--space-sm);">
                    Mosaic CORE Grants 
    ...
    return `
            <div class="mb-lg">
                <h4 class="mb-md" style="color: var(--color-mosaic); border-bottom: 2px solid var(--color-mosaic-light); padding-bottom: var(--space-sm);">
                    ...
                    ${isStatewide ? '<span class="badge" style="background: var(--color-mosaic-badge); color: var(--color-mosaic-dark); margin-left:0.5rem;">Statewide</span>' : ''}
                </h4>
                <div class="rize-grant-list">
