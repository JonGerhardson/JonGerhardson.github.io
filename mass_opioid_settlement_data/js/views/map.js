...
export default {
  ...
  renderMap(geojson) {
    ...
    geojson.features.forEach(feature => {
      ...
      paths.forEach(pathD => {
        ...
        const utilization = entity ? `${Data.formatPercent(entity.pct_spent)} utilized` : 'No data';
  ...
  showTooltip(event, name, entity) {
    ...
    tooltipTimeout = setTimeout(() => {
      if (!entity) {
        tooltip.innerHTML = `<strong>${escapeHtml(name)}</strong><br><span style="color: var(--color-text-muted);">No data available</span>`;
      } else {
        ...
        tooltip.innerHTML = `
                  ...
                  <span style="font-weight: 600; color: ${utilized >= 50 ? 'var(--color-success)' : 'var(--color-text-secondary)'};">
                      ${Data.formatPercent(utilized)} utilized
                  </span>
              `;
      }
